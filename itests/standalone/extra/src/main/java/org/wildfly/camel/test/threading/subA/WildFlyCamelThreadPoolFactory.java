...
/**
 * Custom ThreadPoolFactory.
 *
 * Implementation is mostly identical to the Camel DefaultThreadPoolFactory
 * but this class uses the container ManagedThreadFactory
 */
public class WildFlyCamelThreadPoolFactory implements ThreadPoolFactory {

    @Resource
    private ManagedThreadFactory managedThreadFactory;

    public ExecutorService newCachedThreadPool(ThreadFactory threadFactory) {
        return Executors.newCachedThreadPool(managedThreadFactory);
    }
    ...
    public ExecutorService newThreadPool(int corePoolSize, int maxPoolSize,
                 long keepAliveTime, TimeUnit timeUnit, int maxQueueSize, boolean allowCoreThreadTimeOut, RejectedExecutionHandler
                 rejectedExecutionHandler, ThreadFactory threadFactory) throws IllegalArgumentException {
        if(corePoolSize < 0) {
