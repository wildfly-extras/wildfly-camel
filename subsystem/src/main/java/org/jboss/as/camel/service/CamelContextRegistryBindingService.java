...
public final class CamelContextRegistryBindingService {
    ...
    public static ServiceController<?> addService(final ServiceTarget serviceTarget, ServiceVerificationHandler verificationHandler) {
        ContextNames.BindInfo bindInfo = ContextNames.bindInfoFor(CamelConstants.CAMEL_CONTEXT_REGISTRY_BINDING_NAME);
        BinderService binderService = new BinderService(bindInfo.getBindName()) {
            @Override
            public synchronized void start(StartContext context) throws StartException {
                super.start(context);
                LOGGER.infoBoundNamingService(CamelConstants.CAMEL_CONTEXT_REGISTRY_BINDING_NAME);
            }

            @Override
            public synchronized void stop(StopContext context) {
                super.stop(context);
                LOGGER.debug(CamelConstants.CAMEL_CONTEXT_REGISTRY_BINDING_NAME);
            }
        };
        ...
        builder.addDependency(CamelConstants.CAMEL_CONTEXT_REGISTRY_NAME, CamelContextRegistry.class, new ManagedReferenceInjector<CamelContextRegistry>(injector));
