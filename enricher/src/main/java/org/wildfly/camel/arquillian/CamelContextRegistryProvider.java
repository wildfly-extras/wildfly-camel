...
public class CamelContextRegistryProvider implements ResourceProvider {
    ...
    @Inject
    @SuiteScoped
    private InstanceProducer<CamelContextRegistry> contextRegistryProducer;

    @Inject
    private Instance<CamelContextRegistry> contextRegistry;
    ...
    public Object lookup(ArquillianResource resource, Annotation... qualifiers) {
        if (initialized.compareAndSet(false, true)) {
            ...
            ServiceController<?> service = serviceContainer.getService(CamelConstants.CAMEL_CONTEXT_REGISTRY_NAME);
            CamelContextRegistry registry = service != null ? (CamelContextRegistry) service.getValue() : null;
            contextRegistryProducer.set(registry);
        }
        return contextRegistry.get();
    }
}
